...
!
!    UMAT for Shariff model computes Cauchy stress (stress), spatial elasticity
!     modulus (DDSDDE) (in terms of the Jaumann-rate of the Cauchy stress),
!     and strain energy density (SSE) for a hyperelastic constitutive 
!     model in terms of isochoric principal stretches using explicit 
!     computation of the eigenvectors.
!
!    User input required in "la_sub" and "kstress"
!
!    Stephen Connolly, January 2019.
...
!
      real(8) kprops(5),kE,kl1,kl2,kl3,kl4
!
...
!
      do k1=1,3
      dWdLa(k1) = (kE/labar(k1))*(((2d0/3d0)*log(labar(k1))) +
     1            (kl1*((exp(one-labar(k1)))+labar(k1)-two)) +
     2            (kl2*((exp(labar(k1)-one))-labar(k1))) +
     3            (kl3*(((labar(k1)-one)**3d0)/((labar(k1))**3.6d0))) +
     4            (kl4*((labar(k1)-one)**3d0)))
      end do
!
!    Calculate the first derivative of the constitutive model with
!     respect to the isochoric principal stretch labar(k1)
...
!
         d2WLa2(k1,k2) = ((-kE/(labar(k1)**two))*(((2d0/3d0)
     &       *log(labar(k1)))+(kl1*((exp(one-labar(k1)))+labar(k1)-two))
     &       + (kl2*((exp(labar(k1)-one))-labar(k1))) +
     &       (kl3*(((labar(k1)-one)**3d0)/((labar(k1))**3.6d0))) +
     &       (kl4*((labar(k1)-one)**3d0)))) +
             
     &       ((kE/labar(k1))*((2d0/(3d0*labar(k1))) -
     &       (kl1*((exp(one-labar(k1)))-one)) +
     &       (kl2*((exp(labar(k1)-one))-one)) +
     &       (3d0*kl3*(((labar(k1)-one)**2d0)/((labar(k1))**3.6d0))) -
     &       (3.6d0*kl3*(((labar(k1)-one)**3d0)/((labar(k1))**4.6d0))) +
     &       (3d0*kl4*((labar(k1)-one)**2d0))))
!
        else
!
!    2nd derivative of W with respect to Labar(a) and Labar(b), a /= b
