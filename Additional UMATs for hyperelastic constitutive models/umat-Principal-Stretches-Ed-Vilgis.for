...
!
!    UMAT for the Edwards-Vilgis model to compute the Cauchy stress (stress), 
...
!
      real(8) kprops(4),Nc,Ns,ku,kl,I1,Ibar1
!
...
!
      W = ((Nc/two)*((((one-(kl**two))*Ibar1)/(one-((kl**two)*Ibar1)))
     1    +log(one-((kl**two)*Ibar1))))+((Ns/two)*(((((one-(kl**two))*
     2     (one+ku))/(one-((kl**two)*Ibar1)))*(((la2bar(1))/(one+(ku*
     3     la2bar(1))))+((la2bar(2))/(one+(ku*la2bar(2)))) + ((
     4     la2bar(3))/(one+(ku*la2bar(3))))))+(log(one-((kl**two)
     5     *Ibar1))) + (log(one+(ku*la2bar(1))))+(log(one+(ku*
     6     la2bar(2)))) + (log(one+(ku*la2bar(3))))))
!
...
!
      do k1=1,3
      dWdLa(k1) = Labar(k1)*( 
     &   (Nc*(one+(Ibar1*(kl**four))-(two*(kl**2)))/ 
     &   ((-one+((kl**two)*Ibar1))**two))) + 
     &   Labar(k1)*(Ns*((((kl**two)*(one-(kl**two))*(one+ku))/ 
     &   ((one-((kl**two)*Ibar1))**two))* 
     &   (((la2bar(1))/(one+(ku*la2bar(1))) 
     &   +((la2bar(2))/(one+(ku*la2bar(2))))) 
     &   +((la2bar(3))/(one+(ku*la2bar(3))))))) + 
     &   ((labar(k1)*Ns*(one+ku)*((kl**two)-one))/ 
     &   ((((ku*la2bar(k1))+one)**two)*(-one+((kl**two)*Ibar1)))) + 
     &   ((Ns*ku*labar(k1))/((ku*la2bar(k1))+one)) + 
     &   ((-Ns*(kl**two)*labar(k1))/(one-((kl**two)*Ibar1)))
      end do
!
!    Calculate the first derivative of the constitutive model with
!     respect to the isochoric principal stretch labar(k1)
...
!
        d2WLa2(k1,k2) =
     &   (-Nc*((two*Ibar1*(kl**6)*la2bar(k1))-((Ibar1**2)*(kl**6))-(6d0*
     &   (kl**4)*la2bar(k1))+(three*Ibar1*(kl**4))+(four*(kl**2)*
     &   la2bar(k1))-((kl**2)*Ibar1)-(2*kl**2)+one)/((((kl**2)*Ibar1)
     &   -one)**3))+(((four*Ns*(one+ku)*(one-(kl**2))*(kl**4)*la2bar(k1)
     &   )/((one-(Ibar1*(kl**2)))**3))*(((la2bar(1))/(one+(ku*la2bar(1))
     &   )+ ((la2bar(2))/(one+(ku*la2bar(2)))))+((la2bar(3))/(one+(ku*
     &   la2bar(3))))))-((four*Ns*(one+ku)*(-one+(kl**2))*la2bar(k1)*
     &   (kl**2))/(((one+(la2bar(k1)*ku))**2)*((-one+(Ibar1*(kl**2)))
     &   **2)))+((Ns*(one+ku)*(one-(kl**2))*(kl**2))/((one-(Ibar1*(kl**2
     &   )))**2))*(((la2bar(1))/(one+(ku*la2bar(1)))+ ((la2bar(2))/(one+
     &   (ku*la2bar(2)))))+((la2bar(3))/(one+(ku*la2bar(3)))))-((Ns*(one
     &   +ku)*(-one+(kl**2))*((three*ku*la2bar(k1))-one))/((((ku*
     &   la2bar(k1))+one)**3)*(-one+(Ibar1*(kl**2)))))+((Ns*ku)/((ku*
     &   la2bar(k1))+one))-((two*Ns*(ku**2)*la2bar(k1))/((one+(ku*
     &   la2bar(k1)))**2))+((Ns*(kl**2))/(-one+(Ibar1*(kl**2))))-
     &   ((two*Ns*(kl**4)*(la2bar(k1)))/((-one+(Ibar1*(kl**2)))**2))
!
        else
!
...
!
        d2WLa2(k1,k2) =
     &   ((-two*(kl**2)*Nc*((Ibar1*(kl**4))-(three*(kl**2))+two)*
     &   labar(k1)*labar(k2))/((((kl**2)*Ibar1)-one)**3))+(((four*Ns*
     &   (one+ku)*(one-(kl**2))*(kl**4)*labar(k1)*labar(k2))/((one-
     &   (Ibar1*(kl**2)))**3))*(((la2bar(1))/(one+(ku*la2bar(1)))+((
     &   la2bar(2))/(one+(ku*la2bar(2)))))+((la2bar(3))/(one+(ku*
     &   la2bar(3))))))-((two*Ns*(one+ku)*(-one+(kl**2))*labar(k1)*
     &   labar(k2)*(kl**2))/(((one+(la2bar(k2)*ku))**2)*((-one+(Ibar1*
     &   (kl**2)))**2)))-((two*Ns*(one+ku)*(-one+(kl**2))*labar(k1)*
     &   labar(k2)*(kl**2))/(((one+(la2bar(k1)*ku))**2)*((-one+(Ibar1*
     &   (kl**2)))**2)))-((Ns*two*(kl**4)*labar(k1)*labar(k2))/((-one+
     &   (Ibar1*(kl**2)))**2))
!
        end if
       end do
