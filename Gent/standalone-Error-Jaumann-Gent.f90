...
!
!   Defined in the spatial configuration in terms of the Cauchy stress
...
!
!   This subroutine uses the Gent constitutive model.
!
...
!
      ...
      real(8), dimension(6,6) :: bvdy,bdyb2,b2dyb,bdyId,Iddyb,b2dyb2,&
          Iddyb2,b2dyId,Idy,Id4,bv4,a_vol,a_iso,CSTdyId,IddyCST,ketens
...
!
      W = (-mu*Jm*half)*log(one-((Ibar1-three)/Jm))
!
...
!
      dWdI1 = half*mu*((one-((Ibar1-three)/Jm))**-one)
!
...
!
      dWdI11 = half*mu*(one/Jm)*((one-((Ibar1-three)/Jm))**-two)
!
...
!
!   This subroutine computes the spatial Cauchy stress and elasticity 
!    tensor in terms of the Jaumann rate of the Cauchy stress with the
...
!
      ...
      real(8), dimension(6,6) :: Idy,Id4,a_vol,n11dy11,n22dy11,n33dy11,&
        ...
        n13dy13,n23dy23,a_iso,CSTdyId,IddyCST,KETENS
...
!
      do k1=1,3
      dWdLa(k1) = mu*Labar(k1)*((one-((Ibar1-three)/Jm))**-one)
      end do
!
!    Calculate the first derivative of the constitutive model with
!     respect to the isochoric principal stretch la(k1)
...
!
         d2WLa2(k1,k2) = mu*((one-((Ibar1-three)/Jm))**-one) + &
             two*mu*La2bar(k1)*(one/Jm)*((one-((Ibar1-three)/Jm))**-two)
!
        else
!
!    2nd derivative of W with respect to Labar(a) and Labar(b), a /= b
!
         d2WLa2(k1,k2) = two*mu*Labar(k1)*Labar(k2)*(one/Jm)*&
             ((one-((Ibar1-three)/Jm))**-two)
!
        end if
       end do
      end do
