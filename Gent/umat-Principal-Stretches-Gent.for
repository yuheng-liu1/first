...
!     and strain energy density (SSE) for a Gent hyperelastic constitutive 
...
!
      implicit none
      real(8) J,tol1,kc,kd,la2_max,PI,W,U,kSSE,Ibar1
...
!
     ...
     1       two*mu*La2bar(k1)*(one/Jm)*((one-((Ibar1-three)/Jm))**-two)
!
        else
!
!    2nd derivative of W with respect to Labar(a) and Labar(b), a /= b
...
!
!     First calculate terms present regardless of eigenvalue uniqueness
!
      do k1=1,6
       do k2=1,6
        a_iso(k1,k2) = 
...
!
      la2_max = max(la2(1), la2(2), la2(3))
!
!     If La2(1)=la2(2) AND If La2(2)=la2(3)
!
      if ((abs(la2(1)-la2(2))/(la2_max)).lt.tol1) then
       if ((abs(la2(2)-la2(3))/(la2_max)).lt.tol1) then
        do k1=1,6
         do k2=1,6
...
!     If La2(1)=la2(3)
!
      else if((abs(la2(1)-la2(3))/(la2_max)).lt.tol1) then
       do k1=1,6
        do k2=1,6
          a_iso(k1,k2) = a_iso(k1,k2) + 
...
!     If La2(2)=la2(3)
!
      else if((abs(la2(2)-la2(3))/(la2_max)).lt.tol1) then
       do k1=1,6
        do k2=1,6
          a_iso(k1,k2) = a_iso(k1,k2) + 
     +        ((((B_a(2)*la2(1))-(B_a(1)*la2(2)))/(la2(2)-la2(1)))*
